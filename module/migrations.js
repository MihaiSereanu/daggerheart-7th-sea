...
export class DaggerheartMigrations {
  ...
  static async migrateDocument(document) {
    ...
    if (this.compareVersions(currentVersion, this.CURRENT_VERSION) < 0) {
      console.log(
        `🔄 Migrating ${document.documentName} "${document.name}" from v${currentVersion} to v${this.CURRENT_VERSION}`
      );
      ...
      if (
        this.compareVersions(currentVersion, "1.2.0") < 0 &&
      ...
      if (
        ...
        const weaponItemMigration =
          this._migrateWeaponItemDataStructure(document);
      ...
      if (
        this.compareVersions(currentVersion, "1.2.1") < 0 &&
        document.documentName === "Actor" &&
        document.type === "character"
      ) {
        try {
          const thresholdMigration =
            this._migrateThresholdDataStructure(document);
          ...
          console.error(
            `❌ Error migrating threshold data for "${document.name}":`,
            error
          );
          ui.notifications.error(
            `Migration error for character "${document.name}". Please check the console for details.`
          );
        }
      }

      if (
        this.compareVersions(currentVersion, "1.2.2") < 0 &&
        document.documentName === "Actor" &&
        document.type === "character"
      ) {
        try {
          ...
          console.error(
            `❌ Error fixing zero threshold defaults for "${document.name}":`,
            error
          );
        }
      }
      ...
      if (
        this.compareVersions(currentVersion, "1.2.3") < 0 &&
        document.documentName === "Actor" &&
        document.type === "character"
      ) {
        try {
          const characterLevelModifier =
            this._addCharacterLevelModifier(document);
          ...
          console.error(
            `❌ Error adding character level modifier for "${document.name}":`,
            error
          );
        }
      }
      ...
      if (
        this.compareVersions(currentVersion, "1.2.4") < 0 &&
        document.documentName === "Actor"
      ) {
        try {
          ...
          console.error(
            `❌ Error adding modifier IDs for "${document.name}":`,
            error
          );
        }
      }
      ...
      if (
        this.compareVersions(currentVersion, "1.2.5") < 0 &&
        document.documentName === "Actor"
      ) {
        try {
          const difficultyMigration =
            this._migrateDifficultyToModifierSystem(document);
          ...
          console.error(
            `❌ Error migrating difficulty to modifier system for "${document.name}":`,
            error
          );
        }
      }
      ...
      if (needsUpdate) {
        ...
        console.log(
          `✅ Successfully migrated "${document.name}" to v${this.CURRENT_VERSION}`
        );
      }
    }
  ...
  static _migrateToLocationBased(document) {
    ...
    if (document.documentName === "Item" && !document.system.location) {
      ...
      console.log(
        `📦 Setting location for ${document.type} "${document.name}" → "${location}"`
      );
    }
  ...
  static _migrateWeaponEquipped(document) {
    ...
    if (
      ...
      document.system.equipped === undefined
    ) {
      updates["system.equipped"] = false;
      needsUpdate = true;

      console.log(
        `⚔️ Adding equipped field to weapon "${document.name}" → false`
      );
    }
  ...
  static async migrateWorld() {
    ...
    if (migratedCount > 0) {
      ...
      ui.notifications.info(
        `Migration complete! Updated ${migratedCount} items to new inventory system.`
      );
    } else {
    ...
    if (item.type === "weapon") {
      if (item.system.damage) {
        ...
        if (typeof damage === "string" || typeof damage === "number") {
          ...
          needsUpdate = true;
          console.log(
            `⚔️ Migrating weapon "${item.name}" damage from "${damage}" to structured format`
          );
        }
      } else {
        updateData["system.damage"] = {
        ...
        needsUpdate = true;
        console.log(
          `⚔️ Adding default damage structure to weapon "${item.name}"`
        );
      }
    }
  ...
  static _migrateThresholdDataStructure(actor) {
    ...
    if (actor.system.threshold) {
      ...
      if (threshold.major !== undefined && threshold.major !== null) {
        ...
        if (!isStructured) {
          const originalValue =
            typeof threshold.major === "string"
              ? parseInt(threshold.major) || 0
              : typeof threshold.major === "number"
              ? threshold.major
              : 0;
          ...
          needsUpdate = true;
          console.log(
            `🎯 Migrating major threshold for "${actor.name}" from ${threshold.major} to structured format (${originalValue})`
          );
        }
      } else {
        updateData["system.threshold.major"] =
          this._getDefaultThresholdValues().major;
        needsUpdate = true;
        console.log(
          `🎯 Adding default major threshold for "${actor.name}" (8)`
        );
      }

      if (threshold.severe !== undefined && threshold.severe !== null) {
        const isStructured =
          typeof threshold.severe === "object" &&
        ...
        if (!isStructured) {
          const originalValue =
            typeof threshold.severe === "string"
              ? parseInt(threshold.severe) || 0
              : typeof threshold.severe === "number"
              ? threshold.severe
              : 0;
          ...
          needsUpdate = true;
          console.log(
            `🎯 Migrating severe threshold for "${actor.name}" from ${threshold.severe} to structured format (${originalValue})`
          );
        }
      } else {
        updateData["system.threshold.severe"] =
          this._getDefaultThresholdValues().severe;
        needsUpdate = true;
        console.log(
          `🎯 Adding default severe threshold for "${actor.name}" (12)`
        );
      }
    } else {
      ...
      needsUpdate = true;
      console.log(
        `🎯 Creating default threshold structure for "${actor.name}" (major: 8, severe: 12)`
      );
    }
  ...
  static async _migrateToV121(actor) {
    console.log(
      `Daggerheart | Migrating ${actor.name} to v1.2.1 (weapon slots)`
    );
        ...
        if (item.system.weaponSlot === undefined) {
          if (item.system.equipped) {
            const equippedWeapons = actor.items.filter(
              (i) =>
                i.type === "weapon" && i.system.equipped && i.system.weaponSlot
            );

            const hasPrimary = equippedWeapons.some(
              (w) => w.system.weaponSlot === "primary"
            );
            const hasSecondary = equippedWeapons.some(
              (w) => w.system.weaponSlot === "secondary"
            );
          ...
          if (Object.keys(itemUpdateData).length > 0) {
            ...
            console.log(
              `Daggerheart | Updated weapon ${item.name} with slot data:`,
              itemUpdateData
            );
          }
        }
    ...
    if (actor.system.threshold) {
      ...
      if (threshold.major && typeof threshold.major === "object") {
        if (threshold.major.baseValue === 0 && threshold.major.value === 0) {
          console.log(
            `🔧 Fixing zero major threshold for "${actor.name}" (0 → 8)`
          );
          updateData["system.threshold.major"] =
            this._getDefaultThresholdValues().major;
          needsUpdate = true;
      ...
      if (threshold.severe && typeof threshold.severe === "object") {
        if (threshold.severe.baseValue === 0 && threshold.severe.value === 0) {
          console.log(
            `🔧 Fixing zero severe threshold for "${actor.name}" (0 → 12)`
          );
          updateData["system.threshold.severe"] =
            this._getDefaultThresholdValues().severe;
          needsUpdate = true;
      ...
      if (threshold.major !== undefined && threshold.major !== null) {
        if (typeof threshold.major === "object") {
          if (
            ...
            console.warn(
              `🔧 Fixing corrupted major threshold structure for "${actor.name}"`
            );
            const fallbackValue =
              threshold.major.value ||
              threshold.major.baseValue ||
              this._getDefaultThresholdValues().major.baseValue;
            updateData["system.threshold.major"] = {
              baseValue: fallbackValue,
              modifiers: Array.isArray(threshold.major.modifiers)
                ? threshold.major.modifiers
                : [],
              value: fallbackValue,
            };
            needsUpdate = true;
            ...
            console.warn(
              `🔧 Fixing corrupted major threshold modifiers for "${actor.name}"`
            );
      ...
      if (threshold.severe !== undefined && threshold.severe !== null) {
        if (typeof threshold.severe === "object") {
          if (
            ...
            console.warn(
              `🔧 Fixing corrupted severe threshold structure for "${actor.name}"`
            );
            const fallbackValue =
              threshold.severe.value ||
              threshold.severe.baseValue ||
              this._getDefaultThresholdValues().severe.baseValue;
            updateData["system.threshold.severe"] = {
              baseValue: fallbackValue,
              modifiers: Array.isArray(threshold.severe.modifiers)
                ? threshold.severe.modifiers
                : [],
              value: fallbackValue,
            };
            needsUpdate = true;
            ...
            console.warn(
              `🔧 Fixing corrupted severe threshold modifiers for "${actor.name}"`
            );
  ...
  static async migrateActor(actor) {
    ...
    console.log(
      `Daggerheart | Checking migration for ${actor.name}, current version: ${currentVersion}`
    );
  ...
  static _addCharacterLevelModifier(actor) {
    ...
    if (actor.system.threshold) {
      ...
      if (
        ...
        typeof actor.system.threshold.major === "object"
      ) {
        const majorModifiers = actor.system.threshold.major.modifiers || [];
        const hasCharacterLevelModifier = majorModifiers.some(
        ...
        if (!hasCharacterLevelModifier) {
          ...
          needsUpdate = true;
          console.log(
            `🔧 Adding Character Level modifier (+${level}) to major threshold for "${actor.name}"`
          );
        }
      }
      ...
      if (
        actor.system.threshold.severe &&
        typeof actor.system.threshold.severe === "object"
      ) {
        const severeModifiers = actor.system.threshold.severe.modifiers || [];
        const hasCharacterLevelModifier = severeModifiers.some(
          (mod) => mod.name === modifierName
        );
        ...
        if (!hasCharacterLevelModifier) {
          ...
          needsUpdate = true;
          console.log(
            `🔧 Adding Character Level modifier (+${level}) to severe threshold for "${actor.name}"`
          );
        }
      }
    }
    ...
    for (const fieldPath of modifierFields) {
      ...
      const updatedModifiers = currentData.modifiers.map((modifier) => {
        if (!modifier.id) {
          ...
          console.log(
            `🆔 Adding ID "${newId}" to modifier "${modifier.name}" at ${fieldPath} for "${document.name}"`
          );
      ...
      if (hasPermanentChanges) {
        updateData[`${fieldPath}.permanentModifiers`] = [
          ...existingPermanentModifiers,
          ...newPermanentModifiers,
        ];
        needsUpdate = true;
        console.log(
          `📋 Adding ${newPermanentModifiers.length} permanent modifiers to tracking at ${fieldPath} for "${document.name}"`
        );
      }
    }
  ...
  static _migrateDifficultyToModifierSystem(document) {
    ...
    if (document.type === "environment") {
      ...
      if (
        ...
        typeof difficulty.value === "number"
      ) {
        ...
        if (!difficulty.hasOwnProperty("baseValue")) {
          ...
          needsUpdate = true;
          console.log(
            `🔄 Adding modifier fields to environment difficulty for "${document.name}" (value: ${currentValue})`
          );
        }
        // Case 2: baseValue exists but is wrong (0 when value is not 0)
          ...
          needsUpdate = true;
          console.log(
            `🔧 Fixing incorrect baseValue for environment difficulty "${document.name}" (0 -> ${currentValue})`
          );
        }

        // Always ensure tooltip exists
        if (!difficulty.tooltip) {
          updateData["system.defenses.difficulty.tooltip"] =
            "Difficulty for overcoming this environment.";
          needsUpdate = true;
        }
      }
    }
    ...
    if (document.type === "npc") {
      ...
      if (
        evasion &&
        typeof evasion === "object" &&
        typeof evasion.value === "number"
      ) {
        ...
        if (!evasion.hasOwnProperty("baseValue")) {
          ...
          needsUpdate = true;
          console.log(
            `🔄 Adding modifier fields to NPC evasion for "${document.name}" (value: ${currentValue})`
          );
        }
        // Case 2: baseValue exists but is wrong (0 when value is not 0)
          ...
          needsUpdate = true;
          console.log(
            `🔧 Fixing incorrect baseValue for NPC evasion "${document.name}" (0 -> ${currentValue})`
          );
        }

        // Always ensure tooltip exists
        if (!evasion.tooltip) {
          updateData["system.defenses.evasion.tooltip"] =
