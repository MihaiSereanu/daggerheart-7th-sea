...
export class SimpleActor extends Actor {
  prepareDerivedData() {
    ...
    if (!this.system.defenses["armor-slots"])
      this.system.defenses["armor-slots"] = { value: 0 };

    if (this.system.health?.value !== undefined) {
      this.system.health.value = Math.max(
        0,
        Math.min(this.system.health.value, this.system.health.max || 0)
      );
    }
    if (this.system.stress?.value !== undefined) {
      this.system.stress.value = Math.max(
        0,
        Math.min(this.system.stress.value, this.system.stress.max || 0)
      );
    }
    if (this.system.hope?.value !== undefined) {
      this.system.hope.value = Math.max(
        0,
        Math.min(this.system.hope.value, this.system.hope.max || 0)
      );
    }
    ...
    this.system.barArmor = {
      ...
      min: 0,
      value:
        (this.system.defenses.armor.value || 0) -
        (this.system.defenses["armor-slots"].value || 0),
    };
  ...
  async _handleDeadState() {
    ...
    if (isDying && !hasDeadEffect) {
      ...
      const deadEffectToRemove = this.effects.find((e) =>
        e.statuses.has("dead")
      );
  ...
  getRollData() {
    ...
    data.armor_slots = Math.max(
      0,
      parseInt(data.defenses?.["armor-slots"]?.value) || 0
    );

    data.severe = Math.max(
      0,
      parseInt(data.threshold?.severe?.value ?? data.threshold?.severe) || 0
    );
    data.major = Math.max(
      0,
      parseInt(data.threshold?.major?.value ?? data.threshold?.major) || 0
    );
  ...
  async modifyTokenAttribute(attribute, value, isDelta = false, isBar = true) {
    ...
    const updates = {
      [`system.${attribute}.value`]: Math.clamped(
        current.value + value,
        current.min,
        current.max
      ),
