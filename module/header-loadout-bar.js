...
export class HeaderLoadoutBar {
  constructor(actorSheet) {
    ...
    this.slotTypes = [
      ...
      { key: "ancestry", label: "Heroic Arcana" },
  ...
  _getItemForType(typeKey) {
    return this.actor.items.find(
      (i) => (i.system?.location ?? i.type) === typeKey
    );
  }
  ...
  _activateListeners() {
    ...
    this.container.on("dragover", ".loadout-card-slot", (ev) => {
      ev.preventDefault();
      $(ev.currentTarget).addClass("drag-over");
    });
    this.container.on("dragleave", ".loadout-card-slot", (ev) => {
    ...
    this.container.on("drop", ".loadout-card-slot", async (ev) => {
      ...
      if (this.actor.items.has(item.id)) {
        await this.actor.items
          .get(item.id)
          .update({ "system.location": typeKey });
      } else {
