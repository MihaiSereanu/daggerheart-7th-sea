...
export class EnvironmentActorSheet extends SimpleActorSheet {
  ...
  async getData(options) {
    ...
    context.uiState.categoryStates.actions =
      this.actor.getFlag(
        "daggerheart-7th-sea",
        "uiState.categoryStates.actions"
      ) !== false;
    context.uiState.categoryStates.adversaries =
      this.actor.getFlag(
        "daggerheart-7th-sea",
        "uiState.categoryStates.adversaries"
      ) !== false;

    // Prepare notes HTML for the editor
    context.notesHTML =
      await foundry.applications.ux.TextEditor.implementation.enrichHTML(
        context.systemData.notes,
        {
      ...
      for (const adversaryRef of this.actor.system.potentialAdversaries) {
        try {
          ...
          if (actor) {
            ...
            adversaryData.system.enrichedDescription =
              await foundry.applications.ux.TextEditor.implementation.enrichHTML(
  ...
  async _onAdversaryControl(event) {
    ...
    const actorId =
      event.currentTarget.closest(".adversary-slot").dataset.actorId;
  ...
  async _addAdversary(actor) {
    ...
    const existingAdversary = adversaries.find(
      (adv) => adv.uuid === actor.uuid
    );
    if (existingAdversary) {
      ui.notifications.warn(
        `${actor.name} is already in the potential adversaries list.`
      );
      return;
    }
  ...
  async _removeAdversary(actorId) {
    const adversaries = this.actor.system.potentialAdversaries || [];
    const adversaryToRemove = adversaries.find((adv) =>
      adv.uuid.includes(actorId)
    );
    const updatedAdversaries = adversaries.filter(
      (adv) => !adv.uuid.includes(actorId)
    );
    ...
    if (adversaryToRemove) {
      ui.notifications.info(
        `${adversaryToRemove.name} removed from potential adversaries.`
      );
    }
  }
  ...
  _initializeCategoryStates(html) {
    ...
    categories.forEach((category) => {
      const categoryList = html.find(
        `.item-list[data-location="${category}"], .adversaries-grid[data-location="${category}"]`
      );
      ...
      if (
        this.actor.getFlag(
          "daggerheart-7th-sea",
          `uiState.categoryStates.${category}`
        ) !== false
      ) {
        ...
        if (data) {
          ...
          if (dragData.type === "Actor") {
            ...
            if (actor && (actor.type === "npc" || actor.type === "companion")) {
              ...
              ui.notifications.warn(
                "Only NPCs and Companions can be added as potential adversaries."
              );
            }
          }
        }
