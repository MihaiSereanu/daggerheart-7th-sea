export class EntitySheetHelper {
  ...
  static getAttributeHtml(items, index, group = false) {
    ...
    for (let [key, item] of Object.entries(items)) {
      result =
        result +
        `<input type="${item.type}" name="system.attributes${
          group ? "." + group : ""
  ...
  static async createDialog(data = {}, options = {}) {
    ...
    if (this.TYPES.length > 1) {
      const actorTypes = this.TYPES.filter(
  ...
  static processInlineReferences(formula, actor) {
    ...
    const references = {
      "@prof": () =>
        foundry.utils.getProperty(actor, "system.proficiency.value") ?? 0,
      "@proficiency_value": () =>
        foundry.utils.getProperty(actor, "system.proficiency.value") ?? 0,
      "@lvl": () => foundry.utils.getProperty(actor, "system.level.value") ?? 1,
      "@level_value": () =>
        foundry.utils.getProperty(actor, "system.level.value") ?? 1,
      "@tier": () => foundry.utils.getProperty(actor, "system.tier.value") ?? 1,
      "@agi": () =>
        foundry.utils.getProperty(actor, "system.agility.value") ?? 0,
      "@str": () =>
        foundry.utils.getProperty(actor, "system.strength.value") ?? 0,
      "@ins": () =>
        foundry.utils.getProperty(actor, "system.instinct.value") ?? 0,
      "@pre": () =>
        foundry.utils.getProperty(actor, "system.presence.value") ?? 0,
      "@kno": () =>
        foundry.utils.getProperty(actor, "system.knowledge.value") ?? 0,
      "@hp": () => foundry.utils.getProperty(actor, "system.health.value") ?? 0,
      "@hp_max": () =>
        foundry.utils.getProperty(actor, "system.health.max") ?? 0,
      "@stress_value": () =>
        foundry.utils.getProperty(actor, "system.stress.value") ?? 0,
      "@stress_max": () =>
        foundry.utils.getProperty(actor, "system.stress.max") ?? 0,
      "@hope_value": () =>
        foundry.utils.getProperty(actor, "system.hope.value") ?? 0,
      "@hope_max": () =>
        foundry.utils.getProperty(actor, "system.hope.max") ?? 0,
      "@evasion": () =>
        foundry.utils.getProperty(actor, "system.evasion.value") ?? 0,
      "@armor": () =>
        foundry.utils.getProperty(actor, "system.armor.value") ?? 0,
      "@armor_slots": () =>
        foundry.utils.getProperty(actor, "system.armorSlots.value") ?? 0,
      "@severe": () =>
        foundry.utils.getProperty(actor, "system.severe.value") ?? 0,
      "@major": () =>
        foundry.utils.getProperty(actor, "system.major.value") ?? 0,
      "@minor": () =>
        foundry.utils.getProperty(actor, "system.minor.value") ?? 0,
    };
    ...
    for (const [reference, valueGetter] of Object.entries(references)) {
      if (processedFormula.includes(reference)) {
        try {
          ...
          console.warn(
            `Daggerheart | Failed to process inline reference ${reference}:`,
            error
          );
...
export function buildItemCardChat({
  ...
  const classAttr =
    extraClasses && extraClasses.trim().length
      ? `item-card-chat ${extraClasses.trim()}`
      : "item-card-chat";
  return `
