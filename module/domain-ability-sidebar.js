...
export class DomainAbilitySidebar {
  ...
  _renderSection(slotType, items) {
    const titleHtml = `<div class="domain-abilities-title">${game.i18n.localize(
      slotType.label
    )}</div>`;
  ...
  _renderSectionButtons(slotType, items) {
    ...
    let html = items
      .map(
        (item) => `
        <div class="domain-ability-button" data-item-id="${
          item.id
        }" data-slot-type="${slotType.key}" title="${item.name}">
  ...
  _renderTrackerBubbles(item) {
    ...
    const bubbles = trackers
      ...
      <div class="tracker-notification-bubble" 
           ...
           title="${tracker.name}: ${tracker.value}${
          tracker.maxValue ? "/" + tracker.maxValue : ""
        }">
  ...
  _activateListeners() {
    ...
    this.sidebarElement.on("mousedown", ".domain-ability-button", (ev) => {
      ...
      if (this.previewPinned) {
        ...
        if (
          !this.previewElement ||
          !this.previewElement.hasClass("show") ||
  ...
  async _showPreview(buttonEl) {
    ...
    const description = await TextEditor.enrichHTML(itemData.description, {
    ...
    if (!this.previewElement) {
      this.previewElement = $(
        '<div class="domain-ability-preview"></div>'
      ).appendTo("body");
    }
    ...
    if (left + previewRect.width > window.innerWidth - 10)
      left = rect.left - previewRect.width - 12;
    if (top < 10) top = 10;
  ...
  _pinPreview() {
    ...
    this.previewElement
      .find(".preview-hint")
      .html('<i class="fas fa-mouse"></i> Middle-click to unpin');
  ...
  async _handleDrop(event) {
    ...
    if (existingItems.length >= slotConfig.maxSlots) {
      ui.notifications?.warn(
        `Maximum of ${slotConfig.maxSlots} ${slotConfig.label} items reached.`
      );
      return false;
    }
